...
public class StructureManager
{
    ...
    public StructureData GetStructureByName(string name)
    {
        ...
        return GetRndStruct(12345);
    }


    public StructureData GetRndStruct(int localSeed)
    {
        System.Random random = new System.Random(localSeed);
        int randomIndex = random.Next(structure.structures.Count);
        randomIndex = random.Next(structure.structures.Count);
        randomIndex = random.Next(structure.structures.Count);
        Debug.Log("Спавн структуры под номером " + randomIndex + " Из " + structure.structures.Count);
